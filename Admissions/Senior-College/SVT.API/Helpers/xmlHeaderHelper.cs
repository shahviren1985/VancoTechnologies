        ...
        public static bool BuildWorkbook(string filename,DataTable dt,string Reportname)
        {
            try
            {
                ...
                using (SpreadsheetDocument xl = SpreadsheetDocument.Create(filename, SpreadsheetDocumentType.Workbook))
                {
                    ...
                    string sImagePath = System.Web.Hosting.HostingEnvironment.MapPath(@"~/data/logo.png");
                    ...
                    for (index = 3; index < (dt.Rows.Count + 3); ++index)
                    {
                        ...
                        string fp = @"D:\VancoTechnologies\Code\Admission\Senior-College\SVT.API\data\photos\" + rowdata[10].ToString();
                        InsertImage(ws, index, 16, fp);
