...
function validateInputAndDuplicate(obj) {
    if (obj.value.length == 12) {
        //if server returns true then open alert window to inform user that form with same aadhar number already exists
        AADHAR_EXIST = false;
        $.ajax({
            type: 'GET',
            url: `${BASE_API_HOST}/Form/IsAadharCardExist?aadharNumber=` + obj.value, 
            success: (response) => {
                if (response.isSuccess) {
					if(response.IsFormSubmitted)
					{
					    AADHAR_EXIST = true;  
alert("You have already submitted one form with Aadhar # "+ $('#AadharNumber').val() + ". You can't submit more than one form with same aadhar #. Please contact College office administration team for assistance.");						
										obj.value="";
                    }
					else{
                    if (response.IsAadharExist) {
                        AADHAR_EXIST = true;    
                        if (confirm("You have already submitted the application. Your previous application will be discarded and latest will be considered for admission")) {
                            //alert('You press ok.Form will be replaced');
                        }
                    }
					}
                }
            ...
            err: (err) => {
                console.error("Error while validating Aadhar information");
            }
        })
...
function BlockNonAlphaNumerics(obj, e, allowDecimal, allowNegative) {
    var key;
    var isCtrl = false;
    var keychar;
    var reg;
...
function ExtractAlphaNumeric(obj, decimalPlaces, allowNegative) {
    var temp = obj.value;
    var reg0Str = '[0-9]*';
    if (decimalPlaces > 0) {
        reg0Str += '\\.?[0-9]{0,' + decimalPlaces + '}';
