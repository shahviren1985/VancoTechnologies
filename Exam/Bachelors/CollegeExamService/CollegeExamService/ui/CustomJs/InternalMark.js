...
Examapp.controller("internalMarkEntryCtrl", function ($scope) {
    ...
    $.validate({
        ...
        onSuccess: function ($form) {
            ...
            $('.StudentInternalMark').each(function (e) {
                ...
                var IndexOfArray = findWithAttr($scope.array, "RollNumber", Rollnumber);
                var SetValue = findWithAttr($scope.StudentData, "RollNumber", Rollnumber);
    ...
    $.validate({
        ...
        onSuccess: function ($form) {
            ...
            GetCsvToJsonData('File/Download/Data/SVT?fileName=' + $scope.InternalMark.stream + '-' + $('#course').val() + '_sem' + $('#semester').val() + '_' + CurrentYear + '_' + $scope.InternalMark.examType + '.csv').done(function (dataresponse) {
                ...
                $('#TitleForSubject1').text('Internal Mark For the Subject ' + $("#paper option:selected").text() +'('+$('#paper').val()+')');
