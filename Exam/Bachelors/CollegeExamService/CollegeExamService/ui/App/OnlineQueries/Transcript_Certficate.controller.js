...
Examapp.controller("GenerateTranscriptCertificateCtrl", function ($scope, $http, commonService, MarksheetService) {
        ...
        $http.get(_CommonUr + 'exam/GetStudentsTranscript?CollegeRegistrationNumber=' + prn + '&semester=4')
            .then(function (response) {
                ...
                if (response.status == 200) {
                    ...
                    $scope.TranscriptData.Sem5 = {};
                    $scope.TranscriptData.Sem6 = {};
                    ...
                    $scope.TranscriptData.Sem5.Papers = response.data.ExamTable.filter(x => x.semester == 5);
                    $scope.TranscriptData.Sem6.Papers = response.data.ExamTable.filter(x => x.semester == 6);
    ...
    var ProcessObjArrayToResult = function (transcriptData) {
        for (var pa = 1; pa < 7; pa++) {
            ...
            if (pa == 5)
                transcriptData.Sem5 = angular.copy(ProcessObjArrayToResultSem2(transcriptData.Sem5, pa, transcriptData.AdmissionYear));
            if (pa == 6)
                transcriptData.Sem6 = angular.copy(ProcessObjArrayToResultSem2(transcriptData.Sem6, pa, transcriptData.AdmissionYear));
        }
        transcriptData.TotalOverAllPercentage = (TotalOverAllPercentage / 6);
        transcriptData.OverAllGrade = MarksheetService.GetGrade(transcriptData.TotalOverAllPercentage);
        transcriptData.OverAllGradePOint = TotalOverAllGradePoint / 6
    ...
    var ProcessObjArrayToResultSem2 = function (transcriptData, semester, admissionYear) {
        if (transcriptData.Papers.length > 0) {
            ...
            if (semester < 4 && admissionYear == "2016") {
