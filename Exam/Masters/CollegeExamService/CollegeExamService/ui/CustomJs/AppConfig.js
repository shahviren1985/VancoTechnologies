            ...
            CallAPI("User/GetPaperList?Course=" + $('#stream').val() + "&specialization=" + $('#course').val() + "&sem=" + $('#semester').val() + "_Aggregate").done(function (response) {
                ...
                for (let aj = 0; aj < Records.length; aj++) {
                    try {
                        ...
                        CommonStudentDetails.CollegeRegistrationNumber = Records[ii].College_Registration_No_.replace(new RegExp("'", 'g'), "");
...
Examapp.controller('GenerateAggregateMarksheetCtrl', function ($scope) {
    ...
    //};
    //$scope.GetSemester();
    CallAPI("User/GetPaperList?Course=msc&specialization=Regular&sem=1").done(function (response) {
        var selectedCatrol = $('#Specialization');
        document.getElementById("Specialization").options.length = 0;
        selectedCatrol.append('<option disabled="disabled" selected="selected" value="">---Select----</option>');
        selectedCatrol.prop('disabled', false);
...
Examapp.factory('MarksheetService', ['PaperService', '$http', function (PaperService, $http) {
    ...
    Marksheetservice.GetMarksheetObjectFromCsvObj = function (Obj, PaperObj, HeaderDetail) {
        ...
        StudentObj.Specialisation = HeaderDetail.Semester == "1" && PaperObj.specialisation != "Fashion Design" ? "Regular" : PaperObj.specialisation;
        ...
        StudentObj.College_Registration_No_ = Marksheetservice.RemoveCharfunction(Obj.College_Registration_No_);
        StudentObj.SerialNumber = Obj.SerialNumber;
        StudentObj.Department = PaperService.GetDepartment(PaperObj.specialisation);
    ...
    Marksheetservice.GetMastersGrade = function (strTotalMarkCode) {
        //strTotalMarkCode = Math.round(strTotalMarkCode);
        if (strTotalMarkCode < 50) return 'F';
    ...
    Marksheetservice.GetMasterGradePoint = function (strTotalMarkCode) {
        strTotalMarkCode = Math.round(strTotalMarkCode);
        ...
        if (strTotalMarkCode >= 0 && strTotalMarkCode < 50)
            return "0";
    }
                ...
                try {
                    ...
                    if (CSVObj[strCode] != undefined) {
                        if (CSVObj[strCode] != '' && CSVObj["College_Registration_No_"] != '') {
                            ...
                            if (PaperObj._Status == '')
                                PaperObj._Grade = this.GetMastersGrade(strTotalMarkCode);
                            else
                                PaperObj._Grade = 'F';
                            ...
                            */
                            ...
                            PaperObj._TotalGrade = this.GetMasterGradePoint(PaperObj._FinalTotalMarks);
    ...
    Marksheetservice.GetFinalTotalCalculation = function (MarksheetObjFinal) {
        ...
        MarksheetObjFinal._FinalWeightedPercentage = (MarksheetObjFinal.FailureCount == 0) ? MarksheetObjFinal._FinalWeightedMarks / MarksheetObjFinal._FinalTotalCredit : '-';
        MarksheetObjFinal._FinalPercentageGrade = (MarksheetObjFinal.FailureCount == 0) ? Marksheetservice.GetMastersGrade(MarksheetObjFinal._FinalWeightedPercentage) : '';
...
Examapp.factory('PaperService', function () {
    ...
    PaperService.GetDepartment = function (spe) {
        ...
        if (spe == 'HOSPITALITY AND TOURISM MANAGEMENT' || spe == 'HOSPITALITY & TOURISM MANAGEMENT' || spe == 'INTERIOR DESIGN & RESOURCE MANAGEMENT' || spe == 'INTERIOR DESIGN AND RESOURCE MANAGEMENT') {
            ...
            return spe;
        } else if (spe.trim().toUpperCase() == 'DIABETES & CARDIAC NUTRITION' || spe.trim().toUpperCase() == 'RENAL NUTRITION' || spe.trim().toUpperCase() == 'PEDIATRIC NUTRITION') {
            return 'M.Sc. Specialized Dietetics'
        }
        else if (spe.trim().toUpperCase() == 'FASHION DESIGN') {
            return 'M.Design'
        }
