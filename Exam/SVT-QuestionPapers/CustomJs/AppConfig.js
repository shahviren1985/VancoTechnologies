...
Examapp.controller("ViewPreviousPapersCtrl", function ($scope, $location) {
    function getKeyByValue(object, value) {
        return Object.keys(object).find(key => object[key] === value);
    }
    function findWithAttr(array, attr, value) {
    ...
    $('#SearchPapers').on('click', function () {
        ...
        $.ajax({
            url: _CommonUr + 'Exam/GetExamPapers?examYear=' + year + "&examMonth=" + month + "&semester=" + semester + "&userName=" + userName,
            type: 'get',
            contentType: false,
            processData: false,
            success: function (response) {
                ...
                if(role == "controller")
                    papers = "<table border='1px'><tr><td><b>Serial #</b></td><td><b>Specialisation</b></td><td><b>Paper Title</b></td><td><b>Uploaded By</b></td><td><b>Upload Date</b></td><td><b>Action</b></td><td><b>Random Paper</b></td><td><b>Final Paper</b></td><td><b>Set 1</b></td><td><b>Set 2</b></td><td><b>Set 3</b></td></tr>";
                else
                    papers = "<table border='1px'><tr><td><b>Serial #</b></td><td><b>Specialisation</b></td><td><b>Paper Title</b></td><td><b>Uploaded By</b></td><td><b>Upload Date</b></td><td><b>Set 1</b></td><td><b>Set 2</b></td><td><b>Set 3</b></td></tr>";

                for(var i=0;i<response.length;i++){
                    var fun =  "Download(\"" + response[i].randomSelectedPaper + "\")";
                    var final = response[i].randomSelectedPaper != "" ? "<a onclick='"+fun+"'>Download Paper</a>" : "Not Yet Generated";
                    var uploadDate = new Date(response[i].uploadedDate);
                    var paper1 = response[i].paper1.substr(response[i].paper1.lastIndexOf("/")+1);
                    var paper2 = response[i].paper2.substr(response[i].paper2.lastIndexOf("/")+1);
                    var paper3 = response[i].paper3.substr(response[i].paper3.lastIndexOf("/")+1);
					
					
                    if(role == "controller"){
                        var row = "";
                        if(final == "Not Yet Generated")
                            row ="<tr><td>"+(i+1)+"</td><td>"+response[i].specialisation +"</td><td>"+response[i].paperTitle+"</td><td>"+response[i].uploadedBy+"</td><td>"+uploadDate.getDate()+"-"+months[uploadDate.getMonth()]+"-"+uploadDate.getFullYear()+"</td><td><a onclick='DeletePaper("+response[i].id+")'>Delete</a></td><td><a onclick='GenerateRandomPaper("+response[i].id+")'>Select Paper</a></td><td>"+final+"</td><td><a onclick='Download(\""+response[i].paper1+"\")'>Set 1</a></td><td><a onclick='Download(\""+response[i].paper2+"\")'>Set 2</a></td><td><a onclick='Download(\""+response[i].paper3+"\")'>Set 3</a></td></tr>";
                        else{
                            var diff = (new Date() - uploadDate)/86400000;
                            if(diff > 100){
                                row ="<tr><td>"+(i+1)+"</td><td>"+response[i].specialisation +"</td><td>"+response[i].paperTitle+"</td><td>"+response[i].uploadedBy+"</td><td>"+uploadDate.getDate()+"-"+months[uploadDate.getMonth()]+"-"+uploadDate.getFullYear()+"</td><td>-</td><td>-</td><td>"+final+"</td><td><a onclick='Download(\""+response[i].paper1+"\")'>Set 1</a></td><td><a onclick='Download(\""+response[i].paper2+"\")'>Set 2</a></td><td><a onclick='Download(\""+response[i].paper3+"\")'>Set 3</a></td></tr>";
                            }
                            else{
                                row ="<tr><td>"+(i+1)+"</td><td>"+response[i].specialisation +"</td><td>"+response[i].paperTitle+"</td><td>"+response[i].uploadedBy+"</td><td>"+uploadDate.getDate()+"-"+months[uploadDate.getMonth()]+"-"+uploadDate.getFullYear()+"</td><td>-</td><td>-</td><td>"+final+"</td><td><a style='display:none' onclick='Download(\""+response[i].paper1+"\")'>Set 1</a></td><td><a style='display:none' onclick='Download(\""+response[i].paper2+"\")'>Set 2</a></td><td><a style='display:none' onclick='Download(\""+response[i].paper3+"\")'>Set 3</a></td></tr>";
                            }
                        }
                        papers += row;
                    }
                        ...
                        var row = "<tr><td>"+(i+1)+"</td><td>"+response[i].specialisation +"</td><td>"+response[i].paperTitle+"</td><td>"+response[i].uploadedBy+"</td><td>"+uploadDate.getDate()+"-"+months[uploadDate.getMonth()]+"-"+uploadDate.getFullYear()+"</td><td>"+paper1+"</td><td>"+paper2+"</td><td>"+paper3+"</td></tr>";
                        papers += row;
                    }
                }
...
Examapp.controller("UploadPaperCtrl", function ($scope, $location) {
    function getKeyByValue(object, value) {
        return Object.keys(object).find(key => object[key] === value);
    }
    function findWithAttr(array, attr, value) {
...
Examapp.controller("StudentResultCtrl", ['$scope', 'MarksheetService', 'PaperService', function ($scope, MarksheetService, PaperService) {
    ...
    $('#SeatNumberForms').submit(function (event) {
        ...
        var blockNumbers = ["B-RH6297", "B-RH6299", "B-RH6301", "B-RH6306", "B-RH6307", "B-RR6366", "B-RR6403", "B-RR6404", "B-RR6407", "B-RR6408", "B-RR6411", "B-RR6412"];
        ...
        if (StudentDetailObj != undefined && StudentDetailObj != null) {
            if (StudentDetailObj.Semester == 2 || StudentDetailObj.Semester == 4 || StudentDetailObj.Semester == 6) {
...
Examapp.controller("StudentResultValidationCtrl", ['$scope', 'MarksheetService', 'PaperService', function ($scope, MarksheetService, PaperService) {
    ...
    $('#SeatNumberForms').submit(function (event) {
        ...
        var seatNumber = $.trim($('#txt_SeatNumber').val());
        var blockNumbers = ["B-RH6297", "B-RH6299", "B-RH6301", "B-RH6306", "B-RH6307", "B-RH6222", "B-RH6223", "B-RH6233", "B-RH6239", "B-RR6366", "B-RR6374", "B-RR6403", "B-RR6404", "B-RR6407", "B-RR6408", "B-RR6411", "B-RR6412", "B-RR6401", "B-RR6294", "B-RH6295"];
        if (blockNumbers.includes(seatNumber)) {
            alert("Your result is reserved, please contact Examiniation Department.");
            return;
        }
...
Examapp.controller("YearSettingCtrl", function ($scope) {
    ...
    var ValidateYear = function (enterText) {
...
Examapp.factory('MarksheetService', ['PaperService', '$http', function (PaperService, $http) {
    ...
    Marksheetservice.GetGrade = function (strTotalMarkCode) {
        ...
        else if (strTotalMarkCode >= 40 && strTotalMarkCode < 44) return 'P';
    ...
    Marksheetservice.GetFinalTotalCalculation = function (MarksheetObjFinal) {
        ...
        MarksheetObjFinal._FinalPercentage = (MarksheetObjFinal.FailureCount == 0) ? MarksheetObjFinal.TotalAllSubjectMark / totalSubjects : '-';
...
Examapp.factory('TempMarksheetService', ['PaperService', '$http', function (PaperService, $http) {
    var TempMarksheetService = {};

    TempMarksheetService.GetObjectFromSeatNumber = function (_SeatNumber) {
        var StudentDetailObj = {};
        var R1 = _SeatNumber.toUpperCase().charAt(0);//Course
        var R2 = _SeatNumber.toUpperCase().charAt(2);//Exam Type
        var R3 = _SeatNumber.toUpperCase().charAt(3);//Specialization
    ...
    TempMarksheetService.GetMarksheetObjectFromCsvObj = function (Obj, PaperObj, HeaderDetail) {
        var StudentObj = TempMarksheetService.GetSubjectDetailStudent(Obj, PaperObj, HeaderDetail.Semester);
        StudentObj.FullName = Obj.LastName + ' ' + Obj.FirstName + ' ' + Obj.FatherName + ' ' + Obj.MotherName;
        StudentObj.Course = Obj.Course;
        StudentObj.Specialisation = PaperObj.specialisation;
        StudentObj.PRN = Obj.PRN;
        StudentObj.College_Registration_No_ = Obj.College_Registration_No_;
        StudentObj.Department = PaperService.GetDepartment(StudentObj.Specialisation);
        //StudentObj=MarksheetService.GetFinalTotalCalculation(StudentObj);
        return StudentObj;
    ...
    TempMarksheetService.GetGrade = function (strTotalMarkCode) {
        strTotalMarkCode = Math.round(strTotalMarkCode);
        if (strTotalMarkCode < 40) return 'F';
        else if (strTotalMarkCode >= 40 && strTotalMarkCode < 44) return 'P';
        else if (strTotalMarkCode >= 44 && strTotalMarkCode < 50) return 'C';
        else if (strTotalMarkCode >= 50 && strTotalMarkCode < 55) return 'B';
        else if (strTotalMarkCode >= 55 && strTotalMarkCode < 60) return 'B+';
        else if (strTotalMarkCode >= 60 && strTotalMarkCode < 70) return 'A';
        else if (strTotalMarkCode >= 70 && strTotalMarkCode < 80) return 'A+';
    ...
    TempMarksheetService.GetGradePoint = function (strTotalMarkCode) {
        strTotalMarkCode = Math.round(strTotalMarkCode);
        if (strTotalMarkCode >= 90)
            return "10";
        if (strTotalMarkCode >= 80 && strTotalMarkCode < 90)
        ...
        if (strTotalMarkCode >= 50 && strTotalMarkCode < 55)
            return "5." + ((strTotalMarkCode % 10) + 5) + "0";
        if (strTotalMarkCode >= 45 && strTotalMarkCode < 50)
            return "5." + ((strTotalMarkCode % 5) * 2) + "0";
        if (strTotalMarkCode >= 40 && strTotalMarkCode < 45)
            return "4." + ((strTotalMarkCode % 5) * 2) + "0";
        if (strTotalMarkCode >= 0 && strTotalMarkCode < 40)
            return "0";
    ...
    TempMarksheetService.GetSubjectDetailStudent = function (CSVObj_obj, SpecializationObj, SemesterDetails) {
        try {
            throw CSVObj_obj
        } catch (CSVObj) {
            var StudentDetailObj = {};
            ...
            StudentDetailObj.MissedSubjectFromCSV = 0;
            StudentDetailObj.MissedSubjectFromCSVArray = [];
            if (CSVObj_obj.SeatNumber == '5209' || CSVObj_obj.SeatNumber == 5209 || CSVObj_obj.SeatNumber == '5300' || CSVObj_obj.SeatNumber == 5300) {
                debugger
            }
            for (var propobj = 0; propobj < Object.keys(CSVObj).length; propobj++) {
                try {
                    ...
                    var strExternalTotalCode = "ExternalTotalC" + (prop + 1);
                    var strPracticalMarkCode = "PracticalMarksC" + (prop + 1);
                    //var Grace = 'GraceC' + (prop + 1); temp removed C due to Aggregate marksheet
                    var Grace = 'Grace' + (prop + 1);
                    if (CSVObj[strCode] != undefined) {
                        if (CSVObj[strCode] != '') {
                            var PaperIndex = SpecializationObj.paperDetails.findIndex(p => p.code == CSVObj[strCode]);
                            if (PaperIndex == -1) {
                            ...
                            if (((parseFloat(PaperObj.practicalMaxMarks) || 0) + parseFloat((PaperObj.theoryExternalSection1Max) || 0) + (parseFloat(PaperObj.theoryExternalSection2Max) || 0) + (parseFloat(PaperObj.theoryInternalMax) || 0)) == 50) {
                                PaperObj._IsRequiredDouble = true;
                                PaperObj.DoubleCount = 1;//temp add 1 due to Aggregate marksheet
                            }
                            else if ((parseFloat(PaperObj.practicalMaxMarks) || 0) > 100) {
                                PaperObj.DoubleCount = 1;
                                PaperObj.IsRequiredToMakeCalculate100 = true;
                            ...
                            PaperObj._SubjectCode = CSVObj[strCode];
                            PaperObj._InternalMark = CSVObj[strInternalMarkCode];
                            PaperObj._FloatInternalMark = Math.floor(PaperObj.DoubleCount * (parseFloat(PaperObj._InternalMark) || 0));
                            PaperObj._FloatCredit = (PaperObj.DoubleCount * (parseFloat(PaperObj.credits) || 0));

                            if (SemesterDetails == 1 || SemesterDetails == 2) {
                                PaperObj._Section1Mark = CSVObj[strExternalSection1Code];
                                PaperObj._Section2Mark = CSVObj[strExternalSection2Code];
                                PaperObj._FloatSection1Mark = Math.ceil((PaperObj.DoubleCount * (parseFloat(PaperObj._Section1Mark) || 0)));
                                PaperObj._FloatSection2Mark = Math.ceil((PaperObj.DoubleCount * (parseFloat(PaperObj._Section2Mark) || 0)));
                            } else {
                                var sec1 = CSVObj[strExternalSection1Code];
                                var sec2 = CSVObj[strExternalSection2Code];
                                PaperObj._Section1Mark = ((parseFloat(sec1) || 0) + (parseFloat(sec2) || 0)); //added total as section2 for the sem > 2 only total marks will be there
                                PaperObj._Section2Mark = CSVObj[strExternalTotalCode]; //added total as section2 for the sem > 2 only total marks will be there
                                PaperObj._FloatSection1Mark = Math.ceil((PaperObj.DoubleCount * (parseFloat(PaperObj._Section1Mark) || 0)));
                                PaperObj._FloatSection2Mark = Math.ceil((PaperObj.DoubleCount * (parseFloat(PaperObj._Section2Mark) || 0)));
                            }
                            if (CSVObj[Grace] != undefined) {
                                PaperObj._GraceMark = CSVObj[Grace];
                            } else {
                                var TempGrace = 'GraceC' + (prop + 1)
                                PaperObj._GraceMark = CSVObj[TempGrace];
                            }

                            PaperObj._FloatGraceMark = (PaperObj.DoubleCount * (parseFloat(PaperObj._GraceMark) || 0));

                            PaperObj._PracticalMark = Math.ceil(parseFloat(CSVObj[strPracticalMarkCode]) || 0);
                            PaperObj._FloatPracticalMark = Math.ceil((PaperObj.DoubleCount * (parseFloat(PaperObj._PracticalMark) || 0)));

                            if (parseFloat(PaperObj.practicalMaxMarks) > 100) {
                                PaperObj._PracticalMark = Math.round(((_FloatPracticalMark / (PaperObj.practicalMaxMarks / 50)) * 2));
                                PaperObj.theoryExternalPassing = Math.round(((100 * PaperObj.theoryExternalPassing) / PaperObj.practicalMaxMarks));
                                PaperObj._FloatCredit = Math.round((100 * (parseFloat(PaperObj.credits) || 0)) / PaperObj.practicalMaxMarks)
                                PaperObj.practicalMaxMarks = 100;
                            }

                            PaperObj._ExternalTotal = Math.ceil(PaperObj._FloatSection1Mark + PaperObj._FloatSection2Mark) + PaperObj._FloatGraceMark;
                            PaperObj._FinalTotalMarks = PaperObj._FloatInternalMark + PaperObj._ExternalTotal + PaperObj._FloatPracticalMark;


                            var IsFailInExternal = false;
                            PaperObj._Status = '';
                            if ((PaperObj.DoubleCount * PaperObj.theoryExternalPassing) > 0 && (PaperObj._ExternalTotal + PaperObj._FloatPracticalMark) < (PaperObj.DoubleCount * PaperObj.theoryExternalPassing)) {
                                PaperObj._Status = 'Repeat Final';
                                StudentDetailObj.FailureCount++;
                                IsFailInExternal = true;
                                StudentDetailObj.FailedCredits = parseInt(StudentDetailObj.FailedCredits) + parseInt(PaperObj.credits);
                            }
                            if ((PaperObj.DoubleCount * PaperObj.theoryExternalPassing) > 0 && (PaperObj._FloatInternalMark) < (PaperObj.DoubleCount * PaperObj.theoryInternalPassing)) {
                                PaperObj._Status = 'Repeat Course';
                                if (!IsFailInExternal) {
                                    StudentDetailObj.FailedCredits = parseInt(StudentDetailObj.FailedCredits) + parseInt(PaperObj.credits);
                                    StudentDetailObj.FailureCount++
                            ...
                            //PaperObj._Grade=Marksheetservice.GetGrade(PaperObj._FinalTotalMarks);
                            PaperObj._TotalGrade = TempMarksheetService.GetGradePoint(PaperObj._FinalTotalMarks);
                            PaperObj._TotalGradePoint = (PaperObj._TotalGrade * (PaperObj._FloatCredit));
                            //PaperObj._TotalGradePoint=(PaperObj._TotalGrade*(PaperObj.credits*PaperObj.DoubleCount));
                            StudentDetailObj.TotalAllSubjectMark += (PaperObj._FinalTotalMarks);
                            var NewSubjectObj = {};
        ...
        return TempMarksheetService.GetFinalTotalCalculation(StudentDetailObj);
    }

    TempMarksheetService.GetFinalTotalCalculation = function (MarksheetObjFinal) {
        console.log('**********************RCVD from 1 Object*****************************');
        console.log(MarksheetObjFinal);
        console.log('**********************RCVD from 1 Object*****************************');
        MarksheetObjFinal._FinalTotalCredit = 0;
        ...
        MarksheetObjFinal._FinalPercentage = (MarksheetObjFinal.FailureCount == 0) ? MarksheetObjFinal.TotalAllSubjectMark / MarksheetObjFinal.SubjectDetail.length : '-';
        MarksheetObjFinal._FinalPercentageGrade = (MarksheetObjFinal.FailureCount == 0) ? TempMarksheetService.GetGrade(MarksheetObjFinal._FinalPercentage) : '';
        MarksheetObjFinal._FinalPercentageGradePoint = (MarksheetObjFinal.FailureCount == 0) ? (MarksheetObjFinal._FinalTotalGradePoint / MarksheetObjFinal._FinalTotalCredit) : '-';
        return MarksheetObjFinal;
    }

    return TempMarksheetService;
}]);
