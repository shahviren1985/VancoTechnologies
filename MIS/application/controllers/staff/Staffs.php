...
class Staffs extends CI_Controller {
	...
	public function dashboard() {
		...
		if(isset($_POST['feedback'])) {  
			if (!empty($_POST['filter_feedback_head'])) {
				...
				if($_POST['feedback_type']=='PO'){
					...
					$mystring = "StudentCurriculum";
					if($_POST['specialization']=='All'){
						$mystring = $_POST['stakeholder'].$_POST['feedback_type'];
					}
					...
					if (is_dir($dir)){
						if ($dh = opendir($dir)){
							while (($file = readdir($dh)) !== false){
							   ...
							   if (strpos($filefirst[0], $mystring) !== false){
				...
				elseif ($_POST['feedback_type']=='StudentCurriculum'){
					$mystring = $_POST['feedback_type'].$_POST['Program'];	
					//echo $mystring; die;
					if (is_dir($dir)){
						if ($dh = opendir($dir)){
					...
					if(!empty($arrmatch)){
						foreach ( $arrmatch as $filepath) {
							...
							$danswer_data = json_decode(file_get_contents($fullPathanswer));
							//print_r($danswer_data); die;
							array_push($arrFull, $danswer_data);
						}					
						$data['answer_data'] = $arrFull;
						//print_r($data['answer_data']); die;
				...
				} elseif($teacher=='Parents') { 
					$teacherFeedback="uploads/feedback/$year/question/102_$teacher$st";
					$teacherfeed = file_get_contents($teacherFeedback);
					//var_dump($studentfeed);
					$teacher_data = json_decode($teacherfeed);
					$data['question_data'] = $teacher_data;
				}
				elseif($stringss=='StudentCurriculum') { 
					$teacherFeedback="uploads/feedback/$year/question/102_$stringss$stringfy$st";
					//echo $teacherFeedback; die;
					...
					$fullPath = "uploads/feedback/$year/question/102_$teacher$st";
					//echo $fullPath;
		...
		*/
		 ...
		 $data['question_array']=array();
		 $data['question_option']=array();
		 $data['convert_answer']=array();
		 ...
		 if( isset($_POST['feedback_type']) && isset( $_POST['feedback_year'])  ) {
				...
				if(!empty($question_array)){
					...
					->where('type','radio')
					->where('feedback_id',$question_feedback_id)
			...
			//echo '<pre>';print_r(@$data['question_array']);  die;
			//echo '<pre>';print_r($data['question_option']); echo '</pre>'; die;
			//answer calculation
			
		
			$db_query->from('feedback_answers');
			$db_query->where('year_from',$year_from);
			$db_query->where('year_to',$year_to);
			$db_query->where('feedback_group_id',@$group['id']);
			$answer_array= $db_query->get()->result_array();
			//echo $db_query->last_query();
			//echo '<pre>';print_r($answer_array);  die;
			
			$convert_answer= array();
			
			foreach($answer_array as $ans){
				
				foreach($data['question_option'] as $option){
					if( $ans['answer'] ==  $option['priority']  ){
						 $value= @$convert_answer[@$ans['feedback_id']][@$option['priority']];
						 @$convert_answer[@$ans['feedback_id']][@$option['priority']]= $value+1;
					}
				}
			}
			$data['convert_answer']=@$convert_answer;
			
			$convert_answer_text_radio= array();
			foreach($question_array as $qans){				
				foreach($answer_array as $ans){
						if( ($qans['id']== $ans['feedback_id']) && ( ($qans['type']=='text')  ||  ( $qans['type']=='dropdown' ) )  ){
							$convert_answer_text_radio[$qans['id']][]= $ans['answer'];
						}
				}
			}
				
			//echo '<pre>';print_r($convert_answer_text_radio);  die;	
			
			
			if(@$_POST['feedback']=='excel'){
					$fileName = "feedback";
					$this->load->library('excel');
					$objPHPExcel = new PHPExcel();
					$objPHPExcel->setActiveSheetIndex(0);

					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, 1, 'SNo.');
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, 1, 'Question');
					
					$start=2;
					foreach($data['question_option'] as $qa ){
						
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow( $start, 1, $qa['options']);
						$start++;
					}
					$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow( $start, 1, 'Total Response');
					
					
					 $start_row=0;
					  foreach($data['question_array'] as  $key=>$qa){
						  if($qa['type']!='radio'){ continue; }
						  
						$start_row= $start_row+1;
					    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, ($start_row+1) , $start_row ); 
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow( 1, ($start_row+1) , $qa['question']);  						
                          $count= 0;
						  $column=2;
						 foreach($question_option as $qo){
						   $total= @$convert_answer[@$qa['id']][$qo['priority']]; 
							$count=$count+$total;
							$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow( $column, ($start_row+1) , ($total)>0?$total:0 ); 
							$column++;  
						 }
						 
						 $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow( $column, ($start_row+1) , $count);  
					...
					$db_query->where('year_from',$year_from);
					$db_query->where('year_to',$year_to);
					$db_query->where_in('type',array('text','dropdown'));
					$db_query->where('feedback_group_id',@$group['id']);
					$export_question= $db_query->get()->result_array();					
						
					//echo '<pre>'; echo $db_query->last_query();	print_r($export_question); print_r($convert_answer_text_radio); 	echo '</pre>';	 die;
					...
					foreach($export_question as $key=>$question){
						
						$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($colstart, 1, $question['question']);
						
						 $array= $convert_answer_text_radio[@$question['id']];
						 $keyindex=2;
						 foreach($array as $val){ 
							$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($colstart, $keyindex , $val);
							$keyindex++;
						 }
