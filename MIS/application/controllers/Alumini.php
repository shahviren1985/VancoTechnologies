...
class Alumini extends CI_Controller {
	...
	public function index() {
		$data['transaction_id'] = $this->Alumini_model->get_transaction_number();
		...
		{	
			...
			else
			{
				...
				if($amount == 1025.00){
					...
					$hashstring = $MERCHANT_KEY . '|' . $txnid . '|' . $amount . '|' . $productinfo . '|' . str_replace(' ', '', $firstname) . '|' . $email . '|' . $udf1 . '|' . $udf2 . '|' . $udf3 . '|' . $udf4 . '|' . $udf5 . '||||||' . $SALT;
