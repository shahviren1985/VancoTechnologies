...
class StatusOffline extends CI_Controller {
	...
	public function index() {
		...
		if (isset($postdata ['key'])) {
			...
			$cancellation_payment = $udf1_array[7]; // 0=Full, 1= cancellation payment
	        ...
	        // CONDITION FOR Cancellation Payment
	        if($cancellation_payment==1){
	        	$remark = 'Cancellation Payment';
	        	$postdata['remark'] = $remark;
	        }
			...
			if ($status == 'success'/*   && $resphash == $CalcHashString */) {
				...
				if ($cancellation_payment == 1) {
					...
					$year = (!empty($result2[0]->year))? $result2[0]->year : $year;
						...
						if($status == 'success'){
							...
							if ($partialStudentFlag==1) {
								...
								$message .= '<table>
									...
									<tr><td>Email: </td><td>'.$email.'</td></tr>
									<tr><td>Mobile No.: </td><td>'.$phone.'</td></tr>
									<tr><td>Fee Type: </td><td>Hostel Mess fees payment</td></tr>
									<tr><td>Year/Program: </td><td>'.$year."/".$course_type.'</td></tr>
									<tr><td>Grand Total: </td><td>'.$amount.'</td></tr>
