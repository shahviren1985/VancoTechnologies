<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?> 
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>
		<?php 
			echo $certificate_data['title_string'];		
		?>
		</title>
    </head>
    <style>    	
    	.content-wrapper{
    		align-items: center;
    		text-align: center;
    	}
		table{
			text-transform: uppercase;
		}
		body 
		{
			font-size: 1em;
			line-height:1.375em;
		}
    </style>
    <body>
	<table cellpadding="5" cellspacing ="20" cel style="text-align:center;width:100%;">
	<tr>
		<td>
			<table style="text-align:center;width:100%;margin-left:-5px; vertical-align:top;margin-right:-5px; border:2px solid black;">
				<tr>
					<td style="padding:0 5px">
						<table width="95%" style="margin:10px 0;">
							<tr>
								
								<td colspan="2">
									<img style="height: 80px" src="data:image/png;base64, "  />

									<h2>(Estd. 1959)</h2>
									<br>
									<h2>
										Sir Vithaldas Thackersey College of Home Science (Autonomous)
									</h2>									
									<h2>
										S.N.D.T Women University
									</h2>										
									<h3>
										Sir Vithaldas Vidyavihar, Juhu, Mumbai - 400 049. Tel. 26602504
									</h3>
									<h1>
										LEAVING CERTIFICATE
									</h1>
								</td>
							</tr>							
						</table>						
						<table border="0" width="85%" style="font-size:13px;text-align:left;">
							<!-- <tr>
								<td colspan="2" style="text-align:right;">
									<strong>Date</strong><?php // echo " : ". $certificate_data['certificate_issued_date']; ?>
								</td>
							</tr> -->
							<!-- <tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr> -->
							<tr>
								<td colspan="2" style="text-align:center;">
									<h6>(No change in any entry in this Certificate shall be made except by the authority issuing it and any infringement of this requirement is liable to involve the imposition of penality such as that of rusticattion)</h6>
								</td>
							</tr>	
							<tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>							
							<tr>
								<td width="35%"><strong>1. Registration Number</strong></td>
								<td><?php echo " : ".$certificate_data['registration_number']; ?></td>
							</tr>
							<tr>
								<td><strong>2. Name Of Student</strong> </td>
								<td><?php echo " : ". $certificate_data['name_of_the_student']; ?></td>
							</tr>
							<tr>
								<td><strong>3. Caste And Sub-caste</strong></td>
								<td><?php if($certificate_data['sub_caste']!='' && $certificate_data['sub_caste']!="NULL"){ $subcaste = " - ".$certificate_data['sub_caste']; } else {$subcaste='';}
									 echo " : ". $certificate_data['caste']."".$subcaste." / ".$certificate_data['religion']; ?></td>
							</tr>
							<tr>
								<td><strong>4. Nationality</strong></td>
								<td><?php echo " : ". $certificate_data['nationality']; ?></td>
							</tr>
							<tr>
								<td><strong>5. Date Of Birth</strong></td>
								<td><?php echo " : ". $certificate_data['date_of_birth']; ?></td>
							</tr>
							<tr>
								<td><strong>6. Place Of Birth</strong></td>
								<td><?php echo " : ". $certificate_data['place_of_birth']; ?></td>
							</tr>
							<tr>
								<td><strong>7. Last College Attended</strong></td>
								<td><?php echo " : ". $certificate_data['last_college_attended']; ?></td>
							</tr>
							<tr>
								<td><strong>8. Course Name</strong></td>
								<td><?php echo " : ". $certificate_data['course_name']; ?></td>
							</tr>
							<tr>
								<td><strong>9. Date Of Admission</strong></td>
								<td><?php echo " : ". $certificate_data['date_of_admission']; ?></td>
							</tr>
							<tr>
								<td><strong>10. Date Of Leaving</strong></td>
								<td><?php echo " : ". date('d-M-Y',strtotime($certificate_data['date_of_leaving'])); ?></td>
							</tr>
							<tr>
								<td><strong>11. Area Of Specialization</strong></td>
								<td><?php echo " : ". $certificate_data['area_of_specialization']; ?></td>
							</tr>
							<tr>
								<td><strong>12. Grade</strong></td>
								<td><?php echo " : ". $certificate_data['grade']; ?></td>
							</tr>
							<tr>
								<td><strong>13. Conduct</strong></td>
								<td><?php echo " : ". $certificate_data['conduct']; ?></td>
							</tr>
							<tr>
								<td><strong>14. Reason For Leaving</strong></td>
								<td><?php echo " : ". $certificate_data['reason_for_leaving']; ?></td>
							</tr>
							<tr>
								<td><strong>15. Remarks</strong></td>
								<td><?php echo " : ". $certificate_data['remarks']; ?></td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:center;">
									Certified that the above information is an accordance with the Sr. College Register.
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>
						</table>						
					</td>				
				</tr>			
			</table>
			<table style="width:100%;margin-left:-5px; vertical-align:top;margin-right:-5px;font-size:13px;">
				<tr>
					<td colspan="3">&nbsp;</td>
				</tr>
				<tr>
					<td colspan="3">&nbsp;</td>
				</tr>
				<tr>
					<td colspan="3">&nbsp;</td>
				</tr>
				<tr>
					<td style="text-align:left;"><strong>Date: </strong> <?php echo date('d/m/Y',strtotime($certificate_data['certificate_issued_date'])); ?></td>
					<td style="text-align:center;"><strong>Clerk</strong></td>
					<td style="text-align:center;"><strong>Principal</strong></td>
				</tr>
				<tr>
					<td colspan="3">&nbsp;</td>
				</tr>
			</table>
		</td>
	</tr>

	</table>         	
    </body>
</html>