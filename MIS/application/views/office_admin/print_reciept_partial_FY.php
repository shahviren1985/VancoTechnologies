...
<html lang="en">
	...
	<?php
		...
		$gst = 0;
		$grand_total = 0;
		$caste_category = strtolower(@$fees_form_fields['caste_category']);
		//$roll_number = @$student_details[0]->roll_number;
		$division = @$fees_form_fields['division'];
        $student_name = @$fees_form_fields['first_name'].' '.@$fees_form_fields['last_name'];
        $total_fee = $fees_form_fields['total_fee'];
        $college_fee =  $fees_form_fields['college_fee']*2;
        $online_total = $fees_form_fields['online_total'];
      	
      	$nri = 'No';//@$student_details[0]->nri;
		if($nri){
       ...
       // $course = @$course_details[0]->course_type;
        ...
        $specialization = @$fees_form_fields['specialization'];
        $course_name = @$fees_form_fields['course_name'];
        $program = @$fees_form_fields['program'];
        $year = 'FY';

    ?>
<table cellpadding="5" cellspacing ="20" cel style="text-align:center;width:100%;">
	<tr>
		<td>
						...
						<table width="95%" style="margin:10px 0;">
							<tr>
								<td colspan="2">
									...
									<h2>Sir Vithaldas Thackersey College of Home Science</h2>
										<h2>S.N.D.T Women University,Sir Vithaldas Vidya Vihar,Juhu Road, Santacruz (West) Mumbai - 400 049</h2>
								</table>						
						<table border="0" width="85%" style="font-size:13px;text-align:left;">
							<tr>
								<td colspan="2" style="text-align:center;">
									<?php if ($program == 'master') { ?>
										<p><strong><?php echo $year .' . '. $course_name ?></strong></p>
										<h3>Academic Year - <?php echo date("Y"); ?>-<?php echo date('Y', strtotime('+1 year')); ?></h3><br>		
									<?php }else{ ?>
										<p><strong><?php echo $year .' . '. $specialization ?></strong></p>
										<h3> <?php if($caste_category == 'open'){echo 'Open';} elseif($caste_category == 'reserved') {echo 'Reserved';}?> Category Student<br>Academic Year - <?php echo date("Y"); ?>-<?php echo date('Y', strtotime('+1 year')); ?></h3><br>
									<?php } ?>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td colspan="2" style="text-align:center;">
									<h3>Paid into credit of Sir Vithaldas <br>Thackersey College of Home Science (Autonomous) <?php if ($program == 'master') { ?><br> Self Financed Program <?php } ?></h3>
								</td>
							</tr>
									&nbsp;
							...
							<tr>
								<td colspan="2" style="text-align:center;">
										<h3 style="font-size:13px"><span style="color: red"> 50% INSTALLMENT OF COLLEGE FEES PAID RS. </span> : <?php echo number_format($college_fee/2,'2') ; ?>/-</h3>
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:center;">
							...
							<tr>
								<td colspan="2" style="text-align:center;">	
									<h3 style="font-size:13px"> BALANCE AMOUNT TO BE PAID RS. : <?php echo number_format($college_fee/2,'2') ; ?>/-</h3>
								</td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:center;"><strong>Rupees in words: <?php echo $words ?> ONLY</strong></td>
							</tr>
							<tr>
								<td colspan="2" style="text-align:center;">
									&nbsp;
							...
							<tr>
								<td colspan="2" style="text-align:left;"><strong>Name of Student: </strong> <?php echo $student_name; ?></td>
							</tr>
						<!-- 	<tr>
								<td colspan="2" style="text-align:left;"><strong>Department: </strong><?php echo $specialization; ?></td>
							</tr>
 -->							<tr>
								<td colspan="2" style="text-align:left;"><strong>Specialization: </strong><?php echo $specialization; ?></td>
							</tr>
			...
			<tr>
		        <td style="">
		            <table cellpadding="0" cellspacing="0" style="width: 100%;">
										...
										if($nri == 'Yes'){//if nri
											...
											if($caste_category == 'reserved' && ($category == 'sc' || $category == 'st')) {
												...
												?>
													<tr>
														<td class="borderTable" style="width: 60%; font-weight: bold; font-size: 16px; padding: 5px;"><?php echo $fee_detail->fee_head; ?></td>
														<td style="width: 40%; font-size: 16px; padding: 5px; border-top: 1px solid #000; border-right: 1px solid #000;"><?php if ($fee_detail->fee_head == 'Late Fee'){echo number_format($late_fee);}else{echo number_format($fee_detail->reserved_amount);}?></td>
													</tr>
												<?php }
												}
											}
										...
										if($nri == 'Yes'){//if nri
											...
											if($caste_category == 'reserved' && ($category == 'sc' || $category == 'st')) {
												...
												?>
													<tr>
														<td class="borderTable" style="width: 60%; font-weight: bold; font-size: 16px; padding: 5px;"><?php echo $fee_detail->fee_head; ?></td>
														<td class="borderTable" style="width: 40%; font-size: 16px; padding: 5px;"><?php if ($fee_detail->fee_head == 'Late Fee'){echo number_format($late_fee);}else{echo number_format($fee_detail->reserved_amount);}?></td>
													</tr>
												<?php 
													}
												}
						...
						<tr>
							<?php // ==== For Partial Payment ====
									$fee_head_total = $fee_head_total/2; ?>
						...
						<tr>
							<th style="width: 50%; padding: 10px 10px 10px 0px; border: 1px solid black; font-size: 18px; ">Amount Paid</th>
							<th style="width: 50%; padding: 10px 10px 10px 0px; border: 1px solid black; font-size: 18px; ">₹ <?php 
										$fee_head_total = $fee_head_total/2; 
							 			echo number_format($fee_head_total,2); 
							 		?>							 	
							 </th>
						</tr>
						<tr>
							<th style="width: 50%; padding: 10px 10px 10px 0px; border: 1px solid black; font-size: 18px; ">Balance Amount to be Paid</th>
							<th style="width: 50%; padding: 10px 10px 10px 0px; border: 1px solid black; font-size: 18px; ">₹ <?php echo number_format($fee_head_total,2); 
							 		?>							 	
							 </th>
						</tr>
						<tr>
							<th style="width: 50%; padding: 10px 10px 10px 0px; border: 1px solid black; font-size: 18px; ">Online Payment Processing Charge </th>
							<th style="width: 50%; padding: 10px 10px 10px 0px; border: 1px solid black; font-size: 18px; ">₹ 
								<?php //$pay_percentage = ($fee_head_total/100) * 2;
						...
						<tr>
							<th style="width: 50%; padding: 10px 10px 10px 0px; border: 1px solid black; font-size: 18px; ">Grand Total Paid</th>
							<th style="width: 50%; padding: 10px 10px 10px 0px; border: 1px solid black; font-size: 18px; ">₹ 
								<?php 
								//$grand_total = $fee_head_total+$pay_percentage+$gst;
								$grand_total = $fee_head_total+$online_total;
