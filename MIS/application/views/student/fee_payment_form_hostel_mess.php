...
<?php
 if (count($_GET)>=5){
	...
	// 0=Full, 1= Hostel mess payment
	if(isset($_GET['m'])){
		$hostel_mess_payment = $_GET['m'];
	}else{
		$hostel_mess_payment = 0;
	} 

	if ($hostel_mess_payment == 1) {
		$fee_head_total = 5250;  // 0=Full, 1=Hostel mess payment
		$pay_percentage = 130;
	}
}
		...
		<?php  if (count($_GET)>=5){ ?>
			<form method="post" id="fee_pay" enctype="multipart/form-data" action="<?php echo base_url(); ?>fee_payment_post">
				<div class="card">
						...
						<div class="row">
							<div class="col-md-5">
								...
								<div class="form-group">
									<label for="registration_number">SVT MIS ID: <span class="required-mark">*</span></label>
			        ...
			        <input type="hidden" name="hostel_mess_payment" value="<?php echo $hostel_mess_payment; ?>" required>
