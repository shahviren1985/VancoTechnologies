...
/* 		echo "<pre>"; print_r($update_data); echo "</pre>";
    ...
    public function staff_leave_data(){		
		$database = '';
        $connectionString = $this->session->userdata("connectionString");
        $database = SelectDB($connectionString);
        $db_query = $this->load->database($database, TRUE);
		$db_query->select('*')->from('leave_application')->join('staff_details','staff_details.employee_code = leave_application.staff_id');
		//$db_query->where('leave_from BETWEEN "'.date('Y-m-1').'" AND "'.date('Y-m-16').'"');
		$query = $db_query->get();
		//echo $this->db->get_compiled_select();
		//echo $this->db->last_query(); die('dsdsdsd');
        return $query->result();		
	}
	public function staff_attendance_data(){		
		$database = '';
        $connectionString = $this->session->userdata("connectionString");
        $database = SelectDB($connectionString);
        $db_query = $this->load->database($database, TRUE);
		$db_query->select('employee_code,firstname,lastname,department,type,role,punchin,punchout,bioempcode')
		->from('staff_details')
			 ->join('attendance','staff_details.employee_code = attendance.bioempcode');
		$db_query->where('punchin BETWEEN "2019-12-16" AND "'.date('Y-m-d').'"');
		//$db_query->where('punchin BETWEEN "'.date('Y-m-1').'" AND "'.date('Y-m-16').'"');
		$query = $db_query->get();
		//echo $this->db->get_compiled_select();
		//echo $this->db->last_query(); die('dsdsdsd');
        return $query->result();		
