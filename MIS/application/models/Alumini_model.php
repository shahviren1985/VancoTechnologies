...
class Alumini_model extends CI_Model {
...
/* 	public function insert_transaction($data) {
	...
	public function insert_alumini_registration_data($data) {
		$database = '';
        $connectionString = 'clg_db2';
		$database = SelectDB($connectionString);
        $db_query = $this->load->database($database, TRUE);
		$db_query->insert_batch('alumini_registration_details', $data);
	...
	public function insert_transaction($data) {
		...
		$database = SelectDB($connectionString);
        $db_query = $this->load->database($database, TRUE);
		$query = $db_query->insert_batch('alumini_payment_details', $data);
		return $query;
 	}
	...
	public function get_transaction_number_alumini_registration(){
		$database = '';
        $connectionString = 'clg_db2';
		$database = SelectDB($connectionString);
        $db_query = $this->load->database($database, TRUE);
		$query = $db_query->select('transaction_ref_number')
            ->get('alumini_payment_details');
		return $query->result();	
	}
