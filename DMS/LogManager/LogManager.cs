...
namespace AA.LogManager
{
    public class Logger
    {
        ...
        public Logger(string loggedInUser)
        {
        ...
        public void InitializeStaticMembers()
        {
            try
            {
                ...
                LoggerDetails details = parser.GetLoggerDetails("");
                int logsize = 0;
                int.TryParse(details.LogFileSizeMB, out logsize);
        ...
        public void Error(string className, string methodName, string errorMsg, string logPath)
        {
            ...
            try
            {
                WriteLog(className, "E", methodName, errorMsg, null, null, LoggedInUser);
            }
            catch (Exception ex)
            {
        ...
        public void Error(string className, string methodName, string errorMsg, Exception ex, string logPath)
        {
            ...
            try
            {
                WriteLog(className, "E", methodName, errorMsg, ex, null, LoggedInUser);
            }
            catch (Exception)
            {
        ...
        public void Error(string className, string methodName, Exception ex, string logPath)
        {
            ...
            try
            {
                WriteLog(className, "E", methodName, null, ex, null, LoggedInUser);
            }
            catch (Exception)
            {
                //EventLog.WriteEntry("LogManager_Logger_Error with Exception arguments", ex.Message, EventLogEntryType.Error);
        ...
        public void Error(string className, string methodName, string errorMsg, Hashtable args, string logPath)
        {
            ...
            try
            {
                WriteLog(className, "E", methodName, errorMsg, null, args, LoggedInUser);
            }
            catch (Exception ex)
            {
        ...
        public void Debug(string className, string methodName, string message, string logPath, Hashtable args)
        {
            //LogPath = logPath;
            if (!string.IsNullOrEmpty(logPath))
                LogPath = logPath;
            ...
            try
            {
                WriteLog(className, "D", methodName, message, null, args, LoggedInUser);
            }
            catch (Exception ex)
            {
        ...
        public void Debug(string className, string methodName, string message, Hashtable args)
        {
            try
            {
                WriteLog(className, "D", methodName, message, null, args, LoggedInUser);
        ...
        public void Debug(string className, string methodName, string message, string logPath)
        {
            ...
            try
            {
                WriteLog(className, "D", methodName, message, null, null, LoggedInUser);
            }
            catch (Exception ex)
            {
        ...
        public void Info(string className, string methodName, string message, string logPath)
        {
            ...
            try
            {
                WriteLog(className, "I", methodName, message, null, null, LoggedInUser);
            }
            catch (Exception ex)
            {
        ...
        private void WriteLog(string className, string mode, string methodName, string message, Exception exc, Hashtable args, string loggedInUser)
        {
            try
            {
                lock (lockObject)
                {
                    ...
                    if (exc != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(message))
                        {
                            writeText = System.DateTime.Now.ToString("dd/MMM/yyyy HH:mm:ss") + " || " + mode + " || " + className + " || " + methodName + " || "
                                + message + " || " + arguments + " || by user - " + loggedInUser;
                        }
                        else
                        {
                            writeText = System.DateTime.Now.ToString("dd/MMM/yyyy HH:mm:ss") + " || " + mode + " || " + className + " || " + methodName + " || "
                                + exc.Message + " || " + arguments + " || by user - " + loggedInUser;
                        }
                    }
                    else if (args != null)
                    {
                    ...
                    else if (LogDestination == "db")
                    {
                        ...
                        string _ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings[dbType].ConnectionString;
