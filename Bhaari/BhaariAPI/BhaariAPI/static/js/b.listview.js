...
var JobList = function () {
    ...
    var PrepareJobList = function () {
        ...
        for (var job in JOB_LIST) {
            ...
            item = "<li cid=\"" + JOB_LIST[job].CompanyId + "\" jid=\"" + JOB_LIST[job].Id + "\" onclick=\"LoadJobDetailsPopup(this," + JOB_LIST[job].Id + ",'" + escape(JOB_LIST[job].CompanyName) + "','" + JOB_LIST[job].SourceUrl + "')\">" +
                "<div class=\"job-title\">" + JOB_LIST[job].JobTitle + "</div>" +
                "<div class=\"company-name\">" + JOB_LIST[job].CompanyName + "</div>" +
                "<span class='mark-fav' onclick='return SaveFav(event,\"" + encodeURIComponent(JOB_LIST[job].JobTitle) + "\",\"" + encodeURIComponent(JOB_LIST[job].CompanyName) + "\",\"" + encodeURIComponent(JOB_LIST[job].SourceUrl) + "\",\"" + JOB_LIST[job].Id + "\",\"" + userId + "\",\"" + JOB_LIST[job].DatePosted + "\")'><img src='" + BASE + "static/img/empty-heart.png' /></span>" +
                "</li>";

            if (index == 0) {
                firstItem = item;
                jobId = JOB_LIST[job].Id;
                companyName = JOB_LIST[job].CompanyName;
                url = JOB_LIST[job].SourceUrl;
            }
        ...
        if (!isMobile)
            LoadJobDetailsPopup($(firstItem), jobId, companyName, url);
    ...
    var PrepareLeftNavOptions = function () {
        ...
        list += "<li class=\"active-tab\"><img src=\"" + BASE + "static/img/jobs-icon.png\" width=\"16px\" height=\"16px\" style=\"margin-right: 10px;\" />Jobs</li>";
        list += "<li><img src=\"" + BASE + "static/img/fav-icon.png\" width=\"16px\" height=\"16px\" style=\"margin-right: 10px;\" />Favorite</li>";
...
var LoadJobDetailsPopup = function (jobItem, jobId, companyName, url) {
    ...
    if (previous_val == '' || previous_val != jobId) {
        ...
        if (className == "active-view") {
            ...
            // map view 
            $('#information_div').attr("style", "top: 25%; position: absolute; z-index: 1; right: 20px; width: 400px;top: 25%; position: absolute; z-index: 1; right: 20px; border: 1px solid #AAA; border-radius: 2px; padding: 10px 0px 0px 7px; background-color: #ffffff; width: 400px; min-height: 430px");
        }
...
function SaveFav(event, jobTitle, companyName, sourceUrl, id, userId, datePosted) {
    ...
    if (userId == null || userId == "" || userId == "null") {
        ...
        $(element).attr("src", BASE + "static/img/fav-icon.png");

        $.ajax({
            url: BASE_URL + "savejob?jobId=" + id + "&sourceUrl=" + sourceUrl + "&jobTitle=" + jobTitle + "&companyName=" + companyName + "&userId=" + userId + "&datePosted=" + datePosted,
            success: function (result) {
